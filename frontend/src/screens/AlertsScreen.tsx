...
export const AlertsScreen: React.FC = () => {
  const { alerts, events, acknowledgeAlert, clearAlerts, clearEvents } = useDevice();
  ...
  return (
    <div className="space-y-4 pb-20 select-none">
      <div className="flex items-center justify-between px-1">
        <div>
          <h2 className="text-xl font-black text-white">Alerts & Event Logs</h2>
          <p className="text-xs text-gray-400">Security audit trail & persistence</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          {alerts.length === 0 ? (
            <div className="bg-gray-900/90 border border-gray-800 rounded-3xl p-8 text-center text-gray-500 text-xs">
              ...
              <p className="font-bold text-gray-400 text-sm">No Active Security Alerts</p>
              <p className="mt-1">Tripwires and perimeter sensors are operational.</p>
            </div>
          ) : (
            alerts.map((alert) => (
              <div
                ...
                className={`p-4 rounded-2xl border transition-all ${
                  alert.acknowledged === 0
                    ? 'bg-gray-900/95 border-rose-500/50 shadow-lg shadow-rose-500/10'
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border uppercase ${getSeverityBadge(alert.severity)}`}>
                          {alert.severity}
                        </span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${
                          ...
                          {alert.alert_type === 'INTRUSION' || alert.security_mode === 'AREA' ? 'AREA (PIR)' : 'BELONGING (MPU)'}
                        </span>
                      ...
                      {alert.latitude && alert.longitude && (
                        <div className="flex items-center space-x-1 text-[11px] text-blue-400 mt-1 font-mono">
                          <MapPin className="w-3 h-3" />
                          <span>GPS Location: {alert.latitude.toFixed(5)}, {alert.longitude.toFixed(5)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {alert.acknowledged === 0 && (
                    <button
                      ...
                      className="py-1 px-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-[11px] transition shadow"
                    >
                      Acknowledge
                    </button>
                  )}
      ...
      {activeSubTab === 'history' && (
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between px-1">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">
              Audit Logs ({filteredEvents.length})
            </span>
            {events.length > 0 && (
              <button
                onClick={() => clearEvents()}
              ...
              {filteredEvents.map((evt) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-400 text-xs mt-0.5">{evt.message}</p>
